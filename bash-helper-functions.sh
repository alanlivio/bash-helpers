...
function bhf_script_update() {
  if test -f $SCRIPT_NAME; then
      rm $SCRIPT_NAME;
  fi;
  wget $SCRIPT_URL $SCRIPT_NAME
  if test $? != 0; then test bhf_log_error "wget failed." && return 1 ; fi
...
function bhf_log_print() {
  echo -e "$1" | fold -w100 -s | sed '2~1s/^/  /'
}

function bhf_log_error() {
  bhf_log_print "\033[00;31m-- $* \033[00m"
}

function bhf_log_msg() {
  bhf_log_print "\033[00;33m-- $* \033[00m"
}

function bhf_log_msg_2nd() {
  bhf_log_print "\033[00;33m--   $* \033[00m"
}

function bhf_log_done() {
  bhf_log_print "\033[00;32m-- done\033[00m"
}

function bhf_log_ok() {
  bhf_log_print "\033[00;32m-- ok\033[00m"
}

function bhf_log_try() {
  "$@"
  if test $? -ne 0; then bhf_log_error "$1" && exit 1; fi
}
...
function bhf_test_exist_command() {
  if ! type "$1" &>/dev/null; then
    bhf_log_error "$1 not found."
    return 1
  else
    return 0
  fi
...
function bhf_audio_create_empty() {
...
function bhf_video_create_by_image() {
  : ${1?"Usage: ${FUNCNAME[0]} [image]"}

  ffmpeg -loop_input -i "$1".png -t 5 "$1".mp4
}

function bhf_video_cut() {
...
function bhf_gst_side_by_side_test() {
...
function bhf_gst_side_by_side_args() {
...
function bhf_pygmentize_folder_xml_files_by_extensions_to_jpeg() {
...
function bhf_pygmentize_folder_xml_files_by_extensions_to_rtf() {
  : ${1?"Usage: ${FUNCNAME[0]} [folder]"}
...
function bhf_pygmentize_folder_xml_files_by_extensions_to_html() {
  : ${1?"Usage: ${FUNCNAME[0]} ARGUMENT"}
  bhf_test-exist-command pygmentize
...
function bhf_git_ammend_push_force() {
  git commit -a --amend --no-edit
  git push --force
}

function bhf_git_create_gitignore() {
  : ${1?"Usage: ${FUNCNAME[0]} [contexts,..]"}

  curl -L -s "https://www.gitignore.io/api/$1"
}

function bhf_git_create_gitignore_essentials() {
  bhf_git_create_gitignore code,eclipse,executable,git,intellij,linux,notepadpp,osx,sublimetext,vim,windows,xcode
}

function bhf_git_create_gitignore_javascript() {
  bhf_git_create_gitignore node,bower,grunt
}

function bhf_git_create_gitignore_cpp() {
  bhf_git_create_gitignore c,c++,qt,autotools,make,ninja,cmake
}

function bhf_git_uninstall_reset_clean() {
...
function bhf_git_commit_formated() {
  echo -e "\n" >/tmp/commit.txt
...
function bhf_git_list_large_files() {
...
function bhf_qtcreator_project_from_git() {
  project_name="${PWD##*/}"
...
function bhf_eclipse_list_installed() {
  /opt/eclipse/eclipse \
    -consolelog -noSplash \
...
function bhf_android_start_activity() {
...
function bhf_android_restart_adb() {
  sudo adb kill-server && sudo adb start-server
}

function bhf_android_get_ip() {
  adb shell netcfg
  adb shell ifconfig wlan0
}

function bhf_android_enable_stdout_stderr_output() {
  adb shell stop
  adb shell setprop log.redirect-stdio true
  adb shell start
}

function bhf_android_get_printscreen() {
  adb shell /system/bin/screencap -p /sdcard/screenshot.png
  adb pull /sdcard/screenshot.png screenshot.png
  adb pull /sdcard/screenshot.png screenshot.png
}

function bhf_android_installed_package() {
  : ${1?"Usage: ${FUNCNAME[0]} [package]"}

  adb shell pm list packages | grep $1
}

function bhf_android_uninstall_package() {
  # adb uninstall org.libsdl.app
  : ${1?"Usage: ${FUNCNAME[0]} [package]"}

  adb uninstall $1
}
function bhf_android_install_package() {
  : ${1?"Usage: ${FUNCNAME[0]} [package]"}
...
function bhf_folder_files_sizes() {
  du -ahd 1 | sort -h
}
...
function bhf_image_reconize_text() {
  : ${1?"Usage: ${FUNCNAME[0]} [image]"}

  tesseract -l eng "$1" "$1.txt"
}

function bhf_imagem_compress() {
  : ${1?"Usage: ${FUNCNAME[0]} [image]"}
  bhf_test-exist-command pngquant

  pngquant "$1" --force --quality=70-80 -o "compressed-$1"
}

function bhf_imagem_compress2() {
  : ${1?"Usage: ${FUNCNAME[0]} [image]"}
  bhf_test-exist-command jpegoptim
...
function bhf_pdf_remove_password() {
  : ${1?"Usage: ${FUNCNAME[0]} [pdf]"}
  bhf_test-exist-command qpdf

  qpdf --decrypt "$1" "unlocked-$1"
}

function bhf_pdf_remove_watermark() {
  : ${1?"Usage: ${FUNCNAME[0]} [pdf]"}
...
function bhf_convert_to_markdown() {
  : ${1?"Usage: ${FUNCNAME[0]} [file]"}
  pandoc -s $1 -t markdown -o ${1%.*}.md
}

function bhf_convert_to_pdf() {
  : ${1?"Usage: ${FUNCNAME[0]} [pdf]"}
...
function bhf_rename_to_lowercase_dash() {
  : ${1?"Usage: ${FUNCNAME[0]} [file]"}
...
function bhf_network_open_ports() {
  netstat -tulpn
}

function bhf_network_open_connections() {
  lsof -i
}

function bhf_network_ip() {
  echo "$(hostname -I | cut -d' ' -f1)"
}

function bhf_network_arp_scan() {
  ip=$(bhf_network_ip)
...
function bhf_virtualbox_compact() {
  : ${1?"Usage: ${FUNCNAME[0]} [vdi_file]"}

  VBoxManage modifyhd "$1" compact
}

function bhf_virtualbox_resize_to_2gb() {
  : ${1?"Usage: ${FUNCNAME[0]} [vdi_file"}
...
function bhf_user_reload_bashrc() {
  source ~/.bashrc
}

function bhf_user_permissions_sudo() {
  sudo sh -c 'echo "$USER  ALL=(ALL) NOPASSWD:ALL" >> /etc/sudoers.d/sudoers-user'
}

function bhf_user_permissions_opt() {
...
function bhf_user_permissions_ssh() {
  sudo chmod 700 ~/.ssh/ \
    && sudo chmod 755 ~/.ssh/* \
...
function bhf_user_send_ssh_keys() {
...
function bhf_vscode_run_as_root() {
  : ${1?"Usage: ${FUNCNAME[0]} [folder]"}

  sudo code --user-data-dir="~/.vscode" "$1"
}

function bhf_vscode_install_packages() {
...
function bhf_vscode_uninstall_all_packages() {
  INSTALLED_LIST="$(code --list-extensions)"
...
function bhf_gnome_reset_keybindings() {
...
function bhf_gnome_show_version() {
  gnome-shell --version
...
function bhf_gnome_gdm_restart() {
  sudo /etc/init.d/gdm3 restart
}

function bhf_gnome_settings_reset() {
  : ${1?"Usage: ${FUNCNAME[0]} [scheme]"}

  gsettings reset-recursively $1
}

function bhf_gnome_settings_save_to_file() {
  : ${2?"Usage: ${FUNCNAME[0]} [dconf-dir] [file]"}

  dconf dump $1 >$2
}

function bhf_gnome_settings_load_from_file() {
  : ${1?"Usage: ${FUNCNAME[0]} [dconf-dir] [file]"}

  dconf load $1 <$2
}

function bhf_gnome_settings_diff_actual_and_file() {
  : ${2?"Usage: ${FUNCNAME[0]} [dconf-dir] [file]"}

  TMP_FILE=/tmp/gnome_settings_diff
  bhf_gnome_settings_save-to-file $1 $TMP_FILE
  diff $TMP_FILE $2
}
...
function bhf_vlc_youtube_playlist_extension() {
...
function bhf_npm_install_packages() {
...
function bhf_ruby_install_packages() {
  : ${1?"Usage: ${FUNCNAME[0]} [npm_packages_list]"}
...
function bhf_python_version() {
  python -V 2>&1 | grep -Po '(?<=Python ).{1}'
}

function bhf_python_install_packages() {
  : ${1?"Usage: ${FUNCNAME[0]} [pip3_packages_list]"}

  if ! type pip3 &>/dev/null; then
    bhf_log_error "pip3 not found."
...
function bhf_apt_upgrade() {
  if [ "$(apt list --upgradable 2>/dev/null | wc -l)" -gt 1 ]; then
    sudo apt -y upgrade
  fi
}

function bhf_apt_install_packages() {
...
function bhf_apt_remove_orphan_packages() {
...
function bhf_apt_fetch_install() {
...
function bhf_fetch_extract_to() {
  ...
  if test ! -f /tmp/$FILE_NAME; then
    ...
    wget --continue $1 -P /tmp/
    ret=$?; echo $ret
    test -n $ret && bhf_log_error "wget failed." && return 1
  fi
  ...
  xz)
    ...
    ;;
  *)
    bhf_log_error "$FILE_EXTENSION is not supported compression." && exit
    ;;
  esac
}

function bhf_fetch_youtube_playlist() {
...
function bhf_list_sorted_by_size() {
  du -h | sort -h
}

function bhf_list_recursive_sorted_by_size() {
  du -ah | sort -h
}
...
function bhf_x11_properties_of_window() {
  xprop | grep "^WM_"
}
