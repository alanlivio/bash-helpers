...
function yt_dlp_audio_from_txt() {
    : ${1?"Usage: ${FUNCNAME[0]} <url to audio or list>"}
    yt-dlp --extract-audio --audio-format m4a --batch-file "$1" $BATCH_EXTRA_ARGS
}
...
function yt_dlp_video_480_from_txt() {
    ...
    yt-dlp -f 'best[height<=480]' --recode-video mp4 --batch-file "$1" $BATCH_EXTRA_ARGS
}
