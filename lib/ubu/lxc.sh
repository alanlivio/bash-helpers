...
function bh_ubu_lxc_list_all() {
  lxc image list
  lxc network list
  lxc list
}

function bh_ubu_lxc_image_import() {
  : ${2?"Usage: ${FUNCNAME[0]} <image.tar.gz> <alias>"}
  lxc image import $1 --alias $2
}

function bh_ubu_lxc_profile_assign() {
