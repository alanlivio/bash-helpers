...
function win_install_zotero() {
  win_get_install Zotero.Zotero
}
...
function win_choco_list_installed() {
  choco list -l
}

function win_choco_list_installed_str() {
  powershell -c '
...
function win_choco_install() {
  log_func
  local pkgs_to_install=""
  local pkgs_installed=$(win_choco_list_installed_str)
  for i in "$@"; do
...
function win_choco_clean() {
  log_func
  if ! type choco-cleaner.exe &>/dev/null; then
    win_choco_install choco-cleaner
  fi
